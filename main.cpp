/* program is a Vulkan Grapics Engine with the SDL2 framework, partly adapted and heavily modified from the tutorial from "vulkan-tutorial.com*/
...
const std::string MODEL_PATH = "models/small fighter 2.obj";
...
{	//from SDL Wiki
	...
	window = SDL_CreateWindow("Space Sim", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 960, 540, SDL_WINDOW_VULKAN);
	if (!window)
	{
	...
	{
		std::cout << "Could not find number of extensions" << std::endl;
		return 1;
	}
	...
	{
		std::cout << "Could not find additional extensions" << std::endl;
		return 1;
	}
...
void createLogicalDevice()
{
	...
	std::set<uint32_t> uniqueQueueFamlies = { indices.graphicsFamily.value(), indices.presentFamily.value() };

	float queuePriority = 1.0f;
	for (uint32_t queueFamily : uniqueQueueFamlies)
	{
	...
	VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
	...
	vkGetBufferMemoryRequirements(device, buffer, &mmemRequirements);

	VkMemoryAllocateInfo allocInfo{};
	allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	allocInfo.allocationSize = mmemRequirements.size;
	allocInfo.memoryTypeIndex = findMemoryType(mmemRequirements.memoryTypeBits, properties);
...
void updateUniformBuffer(uint32_t currentImage)
{
	...
	//adapted from tutorial https://programmer.help/blogs/camera-in-opengl.html
	glm::vec3 modelPos = glm::vec3(0.0 + mSpeed[0], 0.0 + mSpeed[1], 50.0 + mSpeed[2]);
	glm::vec3 cameraPos = glm::vec3(0.0f + (move.xp + move.xm), 0.0f + move.yp + move.ym, move.zbuf + (move.zp + move.zm));
	glm::vec3 cameraFront = glm::vec3(0.0f, 0.0f, 1.0f);
	glm::vec3 cameraTarget = glm::vec3((move.xp + move.xm), (move.yp + move.ym), (move.zp + move.zm));
	...
	ubo.model = glm::rotate(ubo.model, glm::radians(180.f), glm::vec3(0.0f, 1.0f, 0.0f));
...
void mainloop()
{
	...
	while (stillRunning)
	{
		drawFrame();
		mSpeed[2] += 0.1;
		/*adapted from http://lazyfoo.net/tutorials/SDL/18_key_states/index.php */
		SDL_Event event;
